...
//		Color.decode("#ff6873");
        ...
        levelPane.setBorder(BorderFactory.createCompoundBorder(BorderFactory.createEmptyBorder(10, 10, 10, 10),
                BorderFactory.createTitledBorder("난이도 선택")));
    ...
    public boolean ketThucGame(int player) {
        ...
        if (player == DuLieuCoVua.MAY) {
            color = (chonCoTrang) ? "백" : "흑";
        } else
            color = (chonCoTrang) ? "흑" : "백";
        if (trangThai == DuLieuCoVua.DACHIEUTUONG) {
            hienThongBaothoat(color + " 체크메이트 ");
            ketThuc = true;
        } else if (trangThai == DuLieuCoVua.THUA) {
            hienThongBaothoat("잘 하셨습니다");
            ketThuc = true;
        }
        return ketThuc;
    }
    ...
    public void hienThongBaothoat(String message) {
        int option = JOptionPane.showOptionDialog(null, message, "게임 종료", 0, JOptionPane.PLAIN_MESSAGE, null,
                new Object[]{"다시 하기", "나가기"}, " 다시 하기");
        if (option == 0) {
...
//		 
    ...
    public void luuLichSuDi() {
        ...
        System.out.println("저장된 이동 수는 " + lichSudichuyen.size() + " 입니다");
        System.out.println("저장된 체스판==============================");
    ...
    public void quaylui() {
        ...
        if (lichSudichuyen.size() >= 1) {
            ...
            System.out.println("되돌린 횟수는" + lichSudichuyen.size() / 2);
        }
    ...
    public void xoa2thangcuoi() {
        ...
        System.out.println("이 체스판을 삭제했습니다");
    ...
    public void thoat() {
        int option = JOptionPane.showConfirmDialog(null, "ㅋ 쫄튀?", "체스",
